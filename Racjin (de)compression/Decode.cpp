...
std::vector<uint8_t> decompress(const std::vector<uint8_t>& buffer, uint32_t decompressedSize) {
	...
	uint8_t lastDecByte = 0; //last decoded byte of the previus decoding iteration
	...
	while (index < buffer.size()) {
		...
		uint16_t nextToken = buffer[index + 1];  //next pair of bytes to decode from the input buffer
		...
		uint32_t seqIndex = destIndex; //start of a byte sequence 
		...
		if ((nextToken & 0x100) != 0) {// bit flag: is nextToken a literal or a reference?
			...
			uint16_t key = ((nextToken >> 3) & 0x1F) + lastDecByte * 32; //0x1F + 0xFF*32 = 8191

			uint32_t srcIndex = seqIndices[key]; //get a reference to a previously decoded sequence
		...
		uint16_t key = frequencies[lastDecByte] + lastDecByte * 32; //0x1F + 0xFF*32 = 8191
